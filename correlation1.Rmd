---
title: Two Quantitative Variables - Correlation
...
If both variables are quantitative, for the first question we can find the `r hl()$fontcolor("correlation")` and for the second we can do a `r hl()$fontcolor("regression")`. 
...
Please note saying `r hl()$fontcolor("x causes y")` is not the same as `r hl()$fontcolor("x by itself determines y")`. 
...
### Case Study: Old Faithful Guyser

[Old Faithful](https://en.wikipedia.org/wiki/Old_Faithful) is a cone geyser located in Yellowstone National Park in Wyoming, United States.
