---
title: Data Collection - Design of Experiments
...
**`r hl()$fontcolor("Stratified Sampling:")` ** First divide population into subgroups, then do a SRS in each subgroup. 
...
**`r hl()$fontcolor("Systematic Sampling:")` ** choose sample according to some deterministic rule. 
...
**`r hl()$fontcolor("Cluster Sampling:")` ** First divide population into subgroups, first do an SRS of these subgroups, then do a SRS in each of those subgroups.
...
An `r hl()$fontcolor("observational study")` observes individuals and measures variables of interest but does not attempt to influence the responses. 

A `r hl()$fontcolor("designed experiment")` deliberately imposes some treatment on individuals in order to observe their responses.
...
The individuals on which an experiment is done are called `r hl()$fontcolor("experimental units")`. When the units are humans they are called `r hl()$fontcolor("subjects")`. A specific experimental condition applied to a unit is called a `r hl()$fontcolor("treatment")`.
...
The problem is this: it was the the subjects themselves who picked their diet (we say they were `r hl()$fontcolor("self-selected")`). But why did subject # 237 pick diet 1 and not 2? If she did it essentially randomly, there is no problem. But maybe there was a tendency for heavier subjects to pick diet 1 (a `r hl()$fontcolor("selection bias")`) and of course heavier people generally lose more weight, and that is why there is a difference between the groups. There is another variable in play, let's call it diet preference, and maybe it is this variable that is the reason for the difference in weight loss.

This type of problem is called `r hl()$fontcolor("confounding")`.
