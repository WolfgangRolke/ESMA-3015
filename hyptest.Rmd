...
`r hl()$fontcolor("null hypothesis has the = sign")` 
...
A `r hl()$fontcolor("complete")` hypothesis test has to have `r hl()$fontcolor("all")` of the following parts:
...
`r hl()$fontcolor("$p < \\alpha \\rightarrow \\text{ reject } H_0$")` 

`r hl()$fontcolor("$p \\ge \\alpha \\rightarrow \\text{ fail to reject } H_0$")` 
...
```{r,echo=FALSE}
x <- 50:65
p <- round(2*(1-pbinom(x, 100, 0.5)), 3)
txt <- data.frame(x, p)
colnames(txt) <- c("Number of Heads", "p value")
kableExtra::kable(txt)
```
...
and we see that $p=0.905>0.05=\alpha$, so we *fail to reject H~0~*.
...
```{r}
test.mean.sim(n=100, mu=50, sigma=10, alpha=0.05)
```

We wanted the test to wrongly reject the null hypothesis $\alpha=5\%$ of the times, it in fact did it $5.46\%$ of the time. Quite close!
...
-  it favours heads over tails a little bit ($\pi=0.55$?)

-  it favours heads over tails a lot ($\pi=0.75$?)

-  it favours heads over tails completely ($\pi=1.0$?)

-  it favours tails over heads a tiny little bit ($\pi=0.48$?)
...
`r hl()$fontcolor("Warning - real live")`
...
`r hl()$fontcolor("Warning - Moodle Quizzes")`
...
So there are two possible mistakes (`r hl()$fontcolor("type I and type II")`) and the probabilities for making them (`r hl()$fontcolor("$\\alpha$ and $\\beta$")`). 
...
`r hl()$fontcolor("if $\\alpha \\downarrow$ then $\\beta \\uparrow$")`
