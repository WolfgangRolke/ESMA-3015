...
**Note** What is and what is not statistically significant is a question of `r hl()$fontcolor("probability")`.
...
To answer the question we would need to do a `r hl()$fontcolor("hypothesis test")`.
`r hl()$vs()`
...
`r hl()$fontcolor("Correlation does not imply Causation ")` 
...
**`r hl()$fontcolor("Population")` **: all of the entities (people, events, things etc.) that are the focus of a study
...
**`r hl()$fontcolor("Census")` **: If all the entities of a population are included in the study.
...
**`r hl()$fontcolor("Sample")` **: any subset of the population
...
**`r hl()$fontcolor("Random sample")` **: a sample found through some randomization (flip of a coin, random numbers on computer etc.)
...
**`r hl()$fontcolor("Simple Random Sample (SRS)")` **: each "entity" in the population has an equal chance of being chosen for the sample. 
...
**`r hl()$fontcolor("Data")` **:  the collection of many pieces of information
...
[Hurricane Maria](https://www.wunderground.com/hurricane/atlantic/2017/hurricane-Maria)

`r hl()$hr()`

**`r hl()$fontcolor("Parameter")` **: any numerical quantity associated with a **population**.
...
**`r hl()$fontcolor("Statistic")` **: any numerical quantity associated with a **sample**.
...
**`r hl()$fontcolor("Statistical Error")` ** the uncertainty in the value of the statistic due to the fact we only used a sample. 
...
**`r hl()$fontcolor("Bias")` ** Any systematic difference between the population and the sample with respect to a variable.
...
Of course we alway want a small variance and a small bias, but if that is not possible, **what is better (if any)?**
...
- We put a cup outside. The cup has marks for each $1/10^{th}$ of an inch. Our data is the number of marks. Values will be 0, 1, 2 etc.
&rarr; quantitative
