...
In Statistcs the mean is important enough to have it's own symbol: $\bar X$ (say: x bar)
...
`r hl()$fontcolor("Does my answer make sense?")`
...
**Median**: the observation "in the middle" of the **ordered** data set:
 ...
 22, 25, 34, 35, 41, 41, 46, `r hl()$fontcolor("46")`, 46, 47, 49, 54, 54, 59, 60
 
What if the Babe had left the Yankees a year earlier?

 25, 34, 35, 41, 41, 46, `r hl()$fontcolor("46, 46")`, 47, 49, 54, 54, 59, 60
...
$$\text{Mean}  \left( x - \overline{ X} \right) = 0$$
...
$$\text{Mean} \left( x - \overline{ X} \right)^2 $$
Another change from the "obvious" is that we should devide this by n-1 instead of n, and with this we have the famous formula for the `r hl()$fontcolor("Variance")`:
...
One problem with having squared everything is that now the units are in "squares". For example, if our data is the age of people, the variance is age^2^.

Usually we want everything in the same units, and this is easy to do by taking square roots, and so we finally have the formula for the `r hl()$fontcolor("Standard Deviation")`:
...
in the discussion of the standard deviation we saw that if we want to compare two sets of numbers, subtracting the mean is a good idea because then the datsets are both centered at 0. Now we go a step further and also divide by the standard deviation, getting to the **z scores**:
...
Of course if the highest possible score was 100 that emans we should expect someone to have gotton 100!

#### **Case Study:** WRInc

let's check whether the empirical rule holds for the income data of the WRInc dataset.
