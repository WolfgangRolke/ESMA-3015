...
This one is a bit more complicated because in theory it might take a very long time until we get a six (though in practise it won't). Let's do some calculations:
...
Usualy we "code" one outcome as $x=0$ (=failure) and the other as $x=1$ (=success) . Then the distribution is given by
...
So if this curve describes the differences in weights (before-after), the people who lost weight are left of 0, so the proability that a randomly selected person on this program looses weight would be this area:
